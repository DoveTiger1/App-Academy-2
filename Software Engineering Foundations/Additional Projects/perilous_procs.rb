...
def filter_out(array, &prc)
    newarr = []
    array.each {|ele| newarr << ele if prc.call(ele) == false}
    newarr
end
...
def first_index(array, &prc)
    array.each_with_index {|ele, i| return i if prc.call(ele) == true}
    nil
end
...
def xnor_select(array, prc_1, prc_2)
    newarr = []
    array.each {|ele| newarr << ele if (prc_1.call(ele) == true && prc_2.call(ele) == true) || (prc_1.call(ele) == false && prc_2.call(ele) == false)}
    newarr
end
...
def filter_out!(array, &prc)
    array.delete_if {|ele| prc.call(ele) == true}
    array
end
...
def chain_map(value, procs)
    newval = value
    procs.each {|ele| newval = ele.call(newval)}
    newval
end
...
def proctition_platinum(array, *prcs)
    ...
    prcs.each_with_index do |prc, idx|
        newarr = []
        array.each do |ele|
            if prc.call(ele) == true && !got.include?(ele)
                newarr << ele
                got << ele
            end
        end
...
def procipher(string, hash)
    sentence = string.split(" ")
    newsentence = []
    sentence.each do |word|
        newword = word
        hash.each_pair {|k, v| newword = v.call(newword) if k.call(word) == true}
        newsentence << newword
    end
    newsentence.join(" ")
end
...
def picky_procipher(string, hash)
    sentence = string.split(" ")
    newsentence = []
    sentence.each do |word|
        newword = word
        ...
        hash.each_pair do |k, v|
            if k.call(word) == true && got == false
                newword = v.call(newword)
                got = true
            end
        end
        newsentence << newword
